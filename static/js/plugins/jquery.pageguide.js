...
tl.pg.init = function(preferences) {
	...
	//hide was put on, so that the texts was not shown upon pageload
	//the class has to be removed, for the numbers to show.
	guide.removeClass('hide');
	
    message.append('<a href="#" class="tlypageguide_close" title="'+lan.pageGuideClose+'">' + lan.Close + '</a>')
      .append('<span></span>')
      .append('<div></div>')
      .append('<a href="#" class="tlypageguide_back" title="Previous">' + lan.Previous + '</a>')
      .append('<a href="#" class="tlypageguide_fwd" title="Next">' + lan.Next + '</a>');

    jQuery('<div/>', {
        'title': lan.pageGuideLaunchTitle,
        'class': 'tlypageguide_toggle'
    }).append(lan.pageGuideLaunch)
      .append('<div><span>' + guide.data('tourtitle') + '</span></div>')
      .append('<a href="javascript:void(0);" title="'+lan.pageGuideClose+'">'+lan.pageGuideClose+' &raquo;</a>').appendTo(wrapper);
