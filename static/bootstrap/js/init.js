    ...
    $('#billingAddTrigger').each(function(){
        ...
        function updatePrices(){
            ...
            for(var i in productsForm.getAllForms()){
                ...
                var orig = $('#product-'+i+'-origAmount').size() > 0 ? $('#product-'+i+'-origAmount').val() : $('#product-'+i+'-Price-PriceAmount-_content').val();

                var t = parsenumber(rate.rate) * parsenumber(orig);
                $('#product-'+i+'-Price-PriceAmount-_content').val(moneyFormat(t));

                //reflect it to linetotal and rewrite to the linetotal
                t = parsenumber($("#product-" + i + "-quantity").val())*t;
                $("#lineTotal-" + i).val(rate.to + ' ' + moneyFormat(t));
            ...
            //set the absolute totals
            $('#total').html(rate.to + ' ' + moneyFormat(total));
            $('#taxTotal').html(rate.to + ' ' + moneyFormat(totalTax));
            $('#allTotal').html(rate.to + ' ' + moneyFormat(total+totalTax));
        }
